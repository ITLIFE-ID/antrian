...
RSpec.describe SatisfactionIndex, type: :model do
  ...
  it { should validate_uniqueness_of(:name).scoped_to(:company_id).case_insensitive.with_message("Nama index kepuasan tidak boleh sama") }
  it { should validate_uniqueness_of(:order_number).scoped_to(:company_id).ignoring_case_sensitivity.with_message("Urutan index kepuasan tidak boleh sama") }
end
